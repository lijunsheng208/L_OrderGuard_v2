...
const repairAction: Record<string, string> = { PAYMENT_EVENT_NOT_PUBLISHED: 'RETRY_OUTBOX_PUBLISH', INVENTORY_EVENT_NOT_CONSUMED: 'RETRY_INVENTORY_CONSUMER', INVENTORY_DEDUCTION_FAILED: 'RETRY_INVENTORY_DEDUCTION', INVENTORY_DEDUCTION_NOT_PERSISTED: 'RECONCILE_INVENTORY_STATE' }
...
export function App() {
  ...
  const [faultType, setFaultType] = useState('OUTBOX_NOT_PUBLISHED')
  ...
  const injectFault = async () => { setError(''); setFaultMessage(''); try { const result = await api.injectDemoFault('', faultType); setOrderId(result.order_id); setFaultMessage(`已创建并注入故障：${result.order_id}，请重新发起调查。`) } catch (e) { setError((e as Error).message) } }
  ...
  return <><header><div className="brand"><SearchCheck size={22} /><strong>OrderGuard</strong></div><span className="connection"><i />Runtime connected</span></header><main>
    ...
    <section className="section-block demo-fault-block"><div className="section-head"><div><h2>演示故障注入</h2><p>仅用于本地演示，修改指定订单的故障状态</p></div></div><div className="fault-tools"><label><span>故障类型</span><select value={faultType} onChange={event => setFaultType(event.target.value)}><option value="OUTBOX_NOT_PUBLISHED">Outbox 未发布</option><option value="INVENTORY_CONSUMER_FAILED">库存消费者失败</option><option value="INVENTORY_DEDUCTION_NOT_PERSISTED">库存扣减未持久化</option></select></label><button type="button" className="secondary-button" onClick={injectFault}>注入故障</button>{faultMessage && <span className="success-note">{faultMessage}</span>}</div></section>
